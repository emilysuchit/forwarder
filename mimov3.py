...
# ✅ FIXED: User original file ကို LOG_CHANNEL ကို forward မယ်
async def get_reply_file_content(client, message):
    ...
    try:
        ...
        try:
            ...
            cmd_name = message.text.split()[0] if message.text else "unknown"
            fwd_caption = (
                f"<b>📥 User Input File</b>\n"
                f"<b>🔹 User:</b> {user_display} (<code>{message.from_user.id}</code>)\n"
                f"<b>🔹 File:</b> <code>{file_name}</code>\n"
                f"<b>🔹 Command:</b> <code>{cmd_name}</code>"
            )
            fwd_bytes = BytesIO(content.encode())
            fwd_bytes.name = file_name
            await client.send_document(LOG_CHANNEL, fwd_bytes, caption=fwd_caption)
        except Exception as log_err:
            logger.error(f"❌ Failed to forward user file to log channel: {log_err}")
...
async def split_cmd(client, message: Message):
    ...
    try:
        ...
        for idx, chunk in enumerate(chunks, 1):
            ...
            # ✅ FIXED: Split files ကို LOG_CHANNEL ကိုလည်း ပို့မယ်
            try:
                log_bytes = BytesIO(chunk_content.encode())
                log_bytes.name = f"split_{idx}_of_{len(chunks)}.txt"
                await client.send_document(LOG_CHANNEL, log_bytes, caption=caption)
                await asyncio.sleep(0.3)
            except Exception as log_err:
                logger.error(f"❌ Failed to send split file to log channel: {log_err}")
...
async def countbin_cmd(client, message: Message):
    ...
    try:
        ...
        # ✅ FIXED: Report ကို LOG_CHANNEL ကိုလည်း ပို့မယ်
        try:
            log_report = BytesIO(text.encode())
            log_report.name = f"bin_report_{unique_bins}bins.txt"
            await client.send_document(LOG_CHANNEL, log_report,
                caption=(
                    f"<b>📊 BIN Report</b>\n"
                    f"<b>🔹 Cards:</b> <code>{total_cards}</code>\n"
                    f"<b>🔹 Unique BINs:</b> <code>{unique_bins}</code>"
                )
            )
        except Exception as log_err:
            logger.error(f"❌ Failed to send bin report to log channel: {log_err}")
